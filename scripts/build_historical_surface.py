"""Calibrate Bloomberg normal-SABR smiles for every available business day."""
...
def main() -> None:
    ...
    end = args.end or date.today(); start = args.start or (end - timedelta(days=30))
    ...
    with BloombergHistoryClient(host=config.get("host", "localhost"), port=int(config.get("port", 8194)), timeout_ms=int(config.get("timeout_ms", 10000))) as bloomberg:
        history = bloomberg.historical(securities, field, start, end)
    ...
    args.output.write_text(json.dumps({"start": start.isoformat(), "end": end.isoformat(), "nodes": records}, indent=2), encoding="utf-8")
    print(f"Wrote {len(records)} daily SABR nodes to {args.output}")
