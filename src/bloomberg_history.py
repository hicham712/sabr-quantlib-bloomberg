...
class BloombergHistoryClient:
    ...
    def __enter__(self):
        ...
        if not self._session.start():
            raise RuntimeError("Could not start Bloomberg Desktop API session")
    ...
    def historical(self, securities: Iterable[str], field: str, start: date, end: date) -> dict[str, dict[str, float]]:
        ...
        while True:
            ...
            if event.eventType() == blpapi.Event.TIMEOUT:
                raise RuntimeError("Bloomberg historical request timed out")
            for message in event:
                if not message.hasElement("securityData"):
                    continue
                sd = message.getElement("securityData")
                ...
                for i in range(rows.numValues()):
                    ...
                    if row.hasElement("date") and row.hasElement(field):
                        ...
                        if value is not None:
                            bloomberg_date = row.getElementAsDatetime("date")
                            result.setdefault(security, {})[self._date_string(bloomberg_date)] = float(value)
