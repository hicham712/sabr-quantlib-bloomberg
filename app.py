...
def load_datasets():
    out={}
    for p in sorted(DATA_DIR.glob("historical_sabr*.json")):
        try:
            payload=json.loads(p.read_text(encoding="utf-8")); nodes=payload.get("nodes",[])
            if not nodes: continue
            name=payload.get("underlying") or payload.get("index") or p.stem.replace("historical_sabr","").strip("_- ") or "Default"
            out[str(name)]=payload
        except (OSError,ValueError):
            continue
    return out

def data_for(underlying):
    ds=load_datasets(); return ds.get(underlying,{"nodes":[]}).get("nodes",[])
...
app.layout=html.Div([
 html.Div([html.Div([html.Div("RATES / VOLATILITY",className="eyebrow"),html.H1("SABR Market Dashboard"),html.Div("Bloomberg normal-volatility surface · weekly historical calibration",className="subtitle")]),html.Div("LIVE DATASET",className="status-pill")],className="header"),
 html.Div([html.Div([html.Label("Underlying"),dcc.Dropdown(id="underlying",clearable=False,className="dark-dropdown")],className="control"),html.Div([html.Label("Calibration date"),dcc.Dropdown(id="date",clearable=False,className="dark-dropdown")],className="control"),html.Div([html.Label("Expiry / maturities"),dcc.Dropdown(id="maturity",multi=True,placeholder="Select one, several, or all",className="dark-dropdown")],className="control maturity-control"),html.Div(id="dataset-info",className="dataset-info")],className="controls"),
 html.Div(id="kpis",className="kpi-row"),html.Div([html.Div("SABR snapshot",className="section-title"),html.Div(id="parameter-table")],className="panel table-panel"),
 html.Div([cc("SMILES · MARKET VS SABR","smile",True),cc("ALPHA","alpha"),cc("BETA","beta"),cc("RHO","rho"),cc("NU","nu"),cc("ATM NORMAL VOLATILITY","atm"),cc("ATM WEEKLY MOVE","atm-move")],className="charts")],className="app-shell")

@app.callback(Output("underlying","options"),Output("underlying","value"),Input("underlying","value"))
def init_underlying(cur):
    ds=load_datasets(); names=sorted(ds)
    if not names:return [],None
    return [{"label":x,"value":x} for x in names],cur if cur in names else names[0]

@app.callback(Output("date","options"),Output("date","value"),Output("dataset-info","children"),Input("underlying","value"),Input("date","value"))
def init_date(underlying,cur):
    nodes=data_for(underlying); d=dates(nodes)
    if not d:return [],None,"NO DATA"
    s=cur if cur in d else d[-1]; p=load_datasets().get(underlying,{})
    return [{"label":x,"value":x} for x in d],s,f"{p.get('frequency','weekly').upper()} · {len(d)} calibration dates · {d[0]} → {d[-1]}"

@app.callback(Output("maturity","options"),Output("maturity","value"),Input("underlying","value"))
def update_maturity(underlying):
    o=mats(data_for(underlying)); return [{"label":x,"value":x} for x in o],o

@app.callback(Output("kpis","children"),Output("parameter-table","children"),Input("underlying","value"),Input("date","value"),Input("maturity","value"))
def snapshot(underlying,d,selected):
    data=data_for(underlying); selected=selected or mats(data); t=date.fromisoformat(d) if d else None; rows=[]
    for m in selected:
...
@app.callback(Output("smile","figure"),Output("alpha","figure"),Output("beta","figure"),Output("rho","figure"),Output("nu","figure"),Output("atm","figure"),Output("atm-move","figure"),Input("underlying","value"),Input("date","value"),Input("maturity","value"))
def graphs(underlying,d,selected):
    rs=data_for(underlying); selected=selected or mats(rs); sm=go.Figure()
