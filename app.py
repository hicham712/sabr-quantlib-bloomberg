...
def init(d):
 n=normalized_nodes();ds=sorted({x['date'] for x in n});es=unique(n,'expiry');ts=unique(n,'swap_tenor');dv=d if d in ds else (ds[-1] if ds else None);return opts(ds),dv,opts(es),es,opts(ts),ts
@app.callback(Output('table','children'),Input('date','value'),Input('expiry','value'),Input('tenor','value'))
def table(d,es,ts):
 n=[x for x in normalized_nodes() if x['date']==d and (not es or x['expiry'] in es) and (not ts or x['swap_tenor'] in ts)]
 if not n:return 'No calibration available.'
 rows=[html.Tr([html.Td(x['expiry']),html.Td(x['swap_tenor']),html.Td(f"{pct(x['alpha']):.3f}%"),html.Td(f"{pct(x['beta']):.2f}%"),html.Td(f"{pct(x['rho']):.2f}%"),html.Td(f"{pct(x['nu']):.2f}%"),html.Td(f"{x['atm_normal_vol']*10000:.2f} bp")]) for x in n]
...
def graphs(d,es,ts):
 n=normalized_nodes();es=es or unique(n,'expiry');ts=ts or unique(n,'swap_tenor');sm=go.Figure();hist={k:go.Figure() for k in ['alpha','beta','rho','nu','atm','move']}
