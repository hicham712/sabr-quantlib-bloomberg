...
def update_maturity(d):
 o=mats(load_data())
 return [{"label":x,"value":x} for x in o],o
...
def snapshot(d,selected):
 selected=selected or mats(load_data())
 data=load_data(); t=date.fromisoformat(d) if d else None
 rows=[]
 for m in selected:
 ...
 r=rows[0][1]
 k=[card("SELECTED MATURITIES",str(len(rows)),", ".join(x[0] for x in rows)),card("ATM NORMAL VOL",f"{float(r['atm_normal_vol'])*10000:.2f} bp",rows[0][0]),card("Alpha",f"{pct(r['alpha']):.3f}%",rows[0][0]),card("Beta",f"{pct(r['beta']):.2f}%",rows[0][0]),card("Rho",f"{pct(r['rho']):.2f}%",rows[0][0])]
 header=html.Thead(html.Tr([html.Th(x) for x in ["EXPIRY","PERIOD","DATE","Alpha","Beta","Rho","Nu","ATM"]]))
 body=[]
